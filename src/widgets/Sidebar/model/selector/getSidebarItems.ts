...
import {
    getRouteAbout, getRouteAdminPanel, getRouteArticles, getRouteMain, getRouteProfile
...
export const getSidebarItemsList = createSelector(getUserAuthData, (userData) => {
    ...
    if (userData) {
        sidebarItemsList.push(
            {
                path: getRouteProfile(userData.id), text: 'Профиль', Icon: Profile, authOnly: true
            },
            {
                path: getRouteAdminPanel(), text: 'Админка', Icon: Profile, authOnly: true
            },
            {
                path: getRouteArticles(), text: 'Статьи', Icon: Article, authOnly: true
            }
        );
    }
